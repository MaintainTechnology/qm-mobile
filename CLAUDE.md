...
React Native + Expo app for **QuoteMax** — primarily the tradie command centre, with explicitly audience-separated public/customer and server-authorised administrator destinations.
...
- **Never guess a price.** Whether a job is firm-price, indicative, inspection-required or a paid
  visit comes from the current server policy for that trade and quote.
- Fees, GST, deposits, subscriptions and limits come from authoritative server/provider state;
  do not hardcode historical marketing claims.

## What this app is

The tradie's phone first: review and approve drafted quotes, watch leads arrive, edit the pricing
book, and track deposits and booked jobs. The current completeness specification additionally
requires native guest/customer capability journeys and an administrator workspace. Customer links
must remain browser-usable without installation; owner, guest/customer and administrator authority
must never be conflated.

## Status

Active parity implementation. Auth, dashboard tabs, quote/trade tools, account/billing/payout
sections and shared native infrastructure exist; `specs/web-mobile-completeness-spec.md` is the
current traceability and acceptance authority. Local green checks are narrower than device,
provider, deployment and cross-tenant release proof.
...
| Connectivity | `@react-native-community/netinfo`                       | drives query online state and explicit offline/cached-data UI                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| Direct DB    | `@supabase/supabase-js` (+ `react-native-url-polyfill`) | **experimental, not the data path.** `src/lib/supabase.ts` — anon key + Clerk token, for realtime/storage trials only. Returns 0 rows on tenant tables until Clerk is registered as a Supabase third-party auth provider and RLS Phase 2 policies exist. Never put a service-role key here                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
| IAP          | `react-native-purchases` (+`-ui`) — RevenueCat          | native SDK integration exists, but charging is fail-closed until server receipt/webhook reconciliation prevents ungranted or duplicate Stripe/store subscriptions                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| Tests        | `jest-expo` + `@testing-library/react-native`           |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| Builds       | EAS Build + EAS Update (`eas.json`)                     | native dirs are generated, not committed                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| Device       | expo-image-picker · expo-local-authentication · expo-notifications · expo-file-system · expo-sharing · expo-document-picker · expo-video · async-storage (+ react-query persister) | camera/photo attach, biometric unlock, push, Bearer-authed downloads (import `expo-file-system/legacy` — the SDK 54 default export is the new File API), PDF/CSV pickers, video playback, offline cache. Deliberately NOT installed: expo-camera (image-picker's system camera suffices), expo-audio (no backend audio endpoint — a mic permission string with no feature invites store review questions). Any new native module requires bumping `expo.version` in the same change (`runtimeVersion.policy: appVersion`) |
| Monitoring   | `@sentry/react-native`                                 | error-only, privacy-scrubbed, no screenshots/view hierarchy/replay/tracing; no-op without the public DSN; source-map credentials stay in the build secret store                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
- `src/lib/env.ts` — `EXPO_PUBLIC_API_URL`. Everything `EXPO_PUBLIC_*` is readable from a shipped
  binary, so it is configuration, never a secret. In `__DEV__` a `localhost` host is rewritten to
  the machine Metro is serving from, so a phone can reach the dev server over the LAN.

### Which backend each build talks to

Both surfaces read the **same live Supabase** (`bobvihqwhtcbxneelfns`) through the same web
`/api/*` routes — the data is never forked. What differs is the **Clerk instance**, and getting it
wrong returns 401 on every `/api/tenant/*` call because the token was signed by the other instance:

| Build                        | API base                    | Clerk instance                        |
| ---------------------------- | --------------------------- | ------------------------------------- |
| dev (Expo Go / dev client)   | local site via `.env.local` | `picked-alien-30…dev` (`pk_test_…`)   |
| EAS `preview` / `production` | `https://quotemax.com.au`   | `clerk.quotemax.com.au` (`pk_live_…`) |

Set per profile in `eas.json` (`build.<profile>.env`), which is the source of truth — EAS's
server-side variables store **one value per name across all environments**, so they cannot express
this split. Accounts do not carry over between the two Clerk instances: a tradie created against
`pk_test` does not exist in production. Use the apex `quotemax.com.au`, not `www.` — the `www` host
307-redirects, costing a round trip on every request.

- `src/lib/api.ts` — typed fetch. Responses are zod-parsed; a shape mismatch throws rather than
  rendering a wrong number.
- `src/lib/ai.ts` — dormant client transport for an authenticated backend stream. It is not a
  shipped assistant merely because the helper exists. There is deliberately no
