---
name: ralph-content
description: Generate high-quality content iteratively with Claude Opus 4.8 powering every generation step — prose/copy generation, a humanization pass on every approved draft to remove AI-tells, and all visual-design steps (SVG diagrams, metaphor selection, validation, iteration) — all via the Vercel AI Gateway. No deterministic scripts.
allowed-tools: Read, Edit, Create, Grep, Glob, LS, WebSearch, Execute, TodoWrite, ToolSearch, mcp__google-drive__search_files, mcp__google-drive__read_file_content, mcp__google-drive__get_file_metadata
user_invocable: true
...
Generate newsletters, lead magnets, LinkedIn posts, and Instagram scripts with iterative quality improvement. Claude Opus 4.8 powers every generation step — prose/copy generation and quality gates, the humanization pass, and all visual-design steps (SVG diagrams, metaphor selection, self-validation, iteration) — all routed through the Vercel AI Gateway. No deterministic scripts, regex parsing, or hard-coded logic.
...
- `--kb` - Enable KB enrichment by agentically querying the NGM Signaling Knowledge Base in Google Drive (Opus 4.8 + the Drive connector). Surfaces cross-domain mechanistic connections. Use for any content touching biological mechanisms, pathways, interventions, or biomarkers. **Auto-enabled for podcast_roundup**
...
### Phase 1.5: Knowledge Base Enrichment (Agentic Google Drive Query) — `--kb` flag
...
**When to use:** Enabled by `--kb` flag. **Auto-enabled for podcast_roundup.** Use for ANY content that touches biological mechanisms, interventions, biomarkers, or clinical protocols.

**Skip when:** Content is purely editorial/opinion, business strategy, or has no clinical/scientific substrate.

**How it works (changed 2026-06):** Phase 1.5 no longer calls an external retrieval pipeline. Instead, **Opus 4.8 queries the KB directly in Google Drive** via the Google Drive MCP connector — searching by topic, reading the full source documents, and synthesizing the cross-domain connections itself. Reading whole KB docs (not retrieval snippets) is what produces sharper, citation-backed insight.

#### The KB in Google Drive

The NGM Signaling Knowledge Base lives in Drive as individual **markdown** (`.md`) and **JSON** (`.json`) documents — one per entity — across four categories: **pathways, interventions, biomarkers, conflicts**.

Each entity doc is richly structured. A pathway doc (e.g. `pathway_pi3k_akt_mtor.md`) contains a Summary, a Mechanistic Context with a **Pathway Crosstalk** table, Clinical/Practical Use Patterns, Dosing and Implementation, an **Evidence and Uncertainty** section graded High/Moderate/Low with DOIs, Alias Normalization, Verifiable Citations, and a **Lossless Structured Appendix** (machine-readable: `core_nodes`, `upstream_triggers`, `downstream_effects`, `interventions_targeting` [effect + confidence + DOI each], `crosstalk` [target_pathway + relationship + confidence + DOI each], `unresolved_uncertainty`). The `crosstalk`, `interventions_targeting`, and `unresolved_uncertainty` sections are the editorial goldmine — they hand you pathway-named, confidence-graded, DOI-backed cross-domain connections directly.

**Canonical folder:** `NGM PIPELINE OUTPUT` — Drive folder ID `1U-aevYGAWJQkoSQ5keSZFKDl4bkkXevb`, with subfolders `pathways` (`156Kn8TX7GSQQX2OZsd8Tvdr3tDVUldjq`), `interventions` (`1_zKUk5NcZc6UUhKPSuKQXI-Gk7Weej4A`), `biomarkers` (`1zMMeVAWBuzDK3JzSaMX6V1v32VjIlXYu`), and `conflicts` (`16C-WULAxHZkhYu5TPQdoNP1G82vhxe4n`).
...
> **Connector quirk (important):** Folder-scoped queries (`parentId = '...'`) come back empty here. **Do not try to enumerate a folder.** Retrieve by `fullText`/`title` search — that is the reliable path.

#### Retrieval loop (agentic)

1. **Extract lookup targets.** From the topic and Phase 1 research, list the specific entities to pull: pathways (e.g. AMPK, mTORC1, NAD+/SIRT1), interventions (drugs, peptides, lifestyle levers), biomarkers, and any named conflicts. Use each entity's common name AND likely aliases — the docs carry alias lists, so several search terms catch the same doc.
2. **Search.** For each target, `search_files` with `fullText contains '<entity>'`; add `title contains '<entity>'` to find the entity's OWN doc rather than passing mentions. Take the top few hits.
3. **Pick the right copy — prefer canonical, then newest.** Multiple KB exports exist in Drive, so the same title can appear more than once. For each entity, prefer the hit whose `parentId` is one of the canonical subfolder IDs above (or whose parent chain leads to `1U-aev...`, checked via `get_file_metadata`); if none is clearly canonical, take the most recently modified (`modifiedTime`). Read each entity once — never read duplicate copies.
4. **Read the whole doc.** `read_file_content` on the 2–5 most relevant entity docs (5+ for a lead magnet or podcast roundup, often 1 for LinkedIn). The crosstalk table and structured appendix carry the cross-domain links — don't stop at the Summary.
5. **Synthesize.** Pull the specific, pathway-named, confidence-graded connections (and their DOIs) from the `crosstalk` / `interventions_targeting` / `unresolved_uncertainty` sections. These become NGM Deep Analysis callouts (below). Carry the DOIs forward as citations.

#### Read the whole document, not snippets

The advantage of querying the KB agentically is that you read the **entire** entity doc, not a retrieval fragment. Always `read_file_content` the full doc and mine its Mechanistic Context, **Pathway Crosstalk** table, and Lossless Structured Appendix (`crosstalk`, `interventions_targeting`, `unresolved_uncertainty`). That is where the specific, non-obvious connections live — a one-line summary or a snippet would miss them. When a topic spans several entities (e.g. exercise → liver GPLD1 → hippocampal neurogenesis, plus the SIRT1/PGC-1α/irisin/BDNF axis), read each entity's doc and let the crosstalk sections connect them.

#### What to look for, by content type

Aim the retrieval loop at the entities and connections each format needs:

- Newsletter: mechanistic depth + specific pathway crosstalk with other longevity pathways (AMPK, mTOR, NAD+, senescence), intervention protocols, biomarker interpretation, unexpected cross-domain connections.
- Lead magnet: intervention protocols and dosing, cross-domain pathway connections, biomarker panels for monitoring, evidence gaps. Pull more entity docs (5+) — lead magnets are the most KB-dense format.
- Podcast roundup: mechanistic depth, pathway crosstalk, intervention protocols, biomarker interpretation, unexpected connections — run the retrieval loop once per finding.
- LinkedIn: the single most surprising cross-domain connection or mechanistic insight that would surprise a knowledgeable clinician.
- Regulatory brief: skip (use Phase 3 verification instead).

#### How to Use KB Output

**Cross-domain connections are where the editorial value lives.** The docs' `crosstalk` and `interventions_targeting` appendices state many of them directly — pathway-named, confidence-graded, DOI-backed. Look for connections like:
...
Run the retrieval loop **separately per finding**, searching for that finding's specific entities. Different findings pull different pathway/intervention/biomarker docs and surface different cross-domain connections. Run the searches in parallel.

#### For LinkedIn posts with `--kb`:

Reading a single entity doc (the most relevant pathway or intervention) is usually enough. Pull the single most surprising cross-domain connection from its crosstalk table and use it as the post's intellectual anchor — the thing that makes someone stop scrolling. Example: "Your patient on metformin may be blunting their own exercise gains — AMPK activation from metformin antagonizes the mTORC1 signal their muscles need for protein synthesis after training."
...
**Goal:** Generate content that passes the quality rubric using Claude Opus 4.8 for prose generation.
...
Your prompt here...
EOF
PROMPT=$(cat /tmp/prompt.txt) && jq -n --arg model "anthropic/claude-opus-4-8" --arg content "$PROMPT" \
...
5. If revisions are needed, call Opus 4.8 again via the gateway with specific revision instructions
...
**Why it runs second, not first:** The Phase 2 self-critique loop locks NGM editorial structure first (evidence cascades, colon technique, opening variety, banned patterns). A separate humanization pass — same model, a dedicated AI-tell-removal prompt — then sands off the residual texture. Doing the humanization before structure is locked produces mush; doing it after produces editorial copy that doesn't read like a model wrote it.

**Model:** Claude Opus 4.8 via the Vercel AI Gateway (`anthropic/claude-opus-4-8`). Use `AI_GATEWAY_API_KEY` from `.env.local` — the same key the Phase 2 generation calls use. This is a separate pass with its own system prompt, not a continuation of the Phase 2 conversation.

**How to call Opus 4.8 for the humanization pass:**
...
jq -n --arg model "anthropic/claude-opus-4-8" --arg system "$SYSTEM" --arg draft "$DRAFT" \
...
1. Take the Phase 2 rubric-approved draft and feed it to Opus 4.8 with the system prompt above.
...
4. **If the scan finds any banned pattern introduced by the humanization pass:**
   ...
   - For 3+ instances: re-call Opus 4.8 with the original draft AND a feedback line listing the specific patterns it introduced, asking for another pass.
5. **Verify content preservation.** Quick spot-check that all named people, dollar figures, percentages, study citations, and the CTA mechanic are still present in the humanized version. If any factual element is missing or altered, re-call Opus 4.8 with an explicit instruction to preserve those specific claims.
...
> 2. `Generate N SVG diagrams via Opus 4.8`
...
>
> **NOTE:** Substring count is a necessary-but-NOT-sufficient check. A fragmented SVG (see "SVG-FRAGMENTATION TRAP" in Phase 5) passes the substring count but renders as an empty box. The Phase 6 publish gate is two-part (static + live-rendering) for this reason. Do not treat the Phase 4 substring check as a final sign-off.
...
EOF
PROMPT=$(cat /tmp/visual_prompt.txt) && jq -n --arg model "anthropic/claude-opus-4-8" --arg content "$PROMPT" \
...
3. For each diagram:
   a. **[Opus 4.8]** Choose the appropriate diagram type (mechanism, comparison, process, etc.)
...
> **🚨 SVG-FRAGMENTATION TRAP — Read before assembling any HTML that embeds SVG diagrams**
>
> Opus-4.8-generated SVGs contain blank lines internally (between `</defs>` and `<rect>`, between element groups, between `<g>` blocks). If your assembly logic splits the source markdown into paragraphs on blank lines and then wraps each paragraph in `<p>...</p>`, the SVG gets fragmented: only the first chunk (up to the first internal blank line) stays inside `<figure>`, and the remaining chunks get wrapped as `<p>SVG-fragment-as-text</p>` siblings. The browser then renders an empty `<svg>` containing only `<defs>` followed by `<p>` tags full of garbled SVG-as-text — and produces an empty box where the diagram should be.
>
> **The 2026-05-20 NAD+ bundle shipped with this exact failure.** All 5 lead-magnet SVGs rendered as empty boxes. `grep -c '<svg'` returned 5 (passing the old publish gate), but `svg.children.length` in the browser returned 1 for every diagram. The user noticed before the gate did.
>
> **MANDATORY assembly pattern when inline SVGs are involved:**
>
> 1. Read the diagram placeholders from the source markdown (e.g. `[DIAGRAM N: ...]`).
> 2. Replace each placeholder with an **opaque single-line token** (e.g. `__SVG_TOKEN_<uuid>__`) surrounded by blank lines, and record `token → <figure>{svg}</figure>` in a map. The token contains NO blank lines, so it survives paragraph splitting as a single paragraph.
> 3. Run your paragraph-splitting + `<p>` wrapping logic on the token-substituted text.
> 4. After all paragraph processing is done, expand each token back to its `<figure class="figure">{full SVG}</figure>` block. Tokens that landed inside a `<p>` paragraph (defensive case) must be replaced inline, not double-wrapped.
> 5. NEVER pass raw SVG content through the paragraph splitter. NEVER inject `<figure>...{multi-line SVG}...</figure>` into text that is about to be split on blank lines.
>
> **Reference implementation:** see `assemble_leadmagnet_v2.py` in the 2026-05-20 NAD+ run output (`.ralph-content/progress.txt` Run 2026-05-20 fix note). The `diagram_to_token` callback and the post-split `token_map` expansion are the load-bearing parts.

### Phase 6: Publish + Learn

**Goal:** Save output and capture learnings.

> **🚨 PUBLISH-TIME GATE — Lead Magnets (two-part: count AND rendering)**
>
> The old gate counted `<svg` substring occurrences. That was insufficient — a fragmented SVG (see "SVG-FRAGMENTATION TRAP" in Phase 5) still passes the substring count even though every diagram renders as an empty box in a browser. As of 2026-05-20, the gate is two-part: a static count gate AND a live-rendering gate.
>
> **Part 1 — Static count + structural check (must pass before continuing):**
>
> ```bash
> SLUG="<your-slug>"
> HTML="content/learn-platform/lead-magnets/${SLUG}.html"
> SVG_COUNT=$(grep -c '<svg' "$HTML" 2>/dev/null || echo 0)
> # Detect SVG fragmentation: any <figure> containing a <p> sibling means an SVG was split mid-content
> STRAY_P_IN_FIGURE=$(python3 -c "
> import re
> html = open('$HTML').read()
> figures = re.findall(r'<figure class=\"figure\">(.*?)</figure>', html, re.DOTALL)
> bad = sum(1 for f in figures if re.search(r'<p[\s>]', f))
> print(bad)
> ")
> if [ "$SVG_COUNT" -lt 3 ]; then
>   echo "❌ COUNT GATE FAILURE: $SVG_COUNT diagrams. Rubric requires ≥3."
>   exit 1
> fi
> if [ "$STRAY_P_IN_FIGURE" -gt 0 ]; then
>   echo "❌ STRUCTURE GATE FAILURE: $STRAY_P_IN_FIGURE figure(s) contain stray <p> tags."
>   echo "   This means SVG fragmentation occurred during assembly."
>   echo "   See 'SVG-FRAGMENTATION TRAP' in Phase 5 — re-assemble with the opaque-token pattern."
>   exit 1
> fi
> echo "✅ Static gate passed: $SVG_COUNT diagrams, 0 stray <p> in <figure>."
> ```
>
> **Part 2 — Live-rendering gate (must run for any lead magnet before declaring complete):**
>
> The substring count + structural check catches the 2026-05-20 fragmentation bug, but cannot catch every rendering failure (malformed XML inside the SVG, invalid path data, etc.). For high-confidence sign-off, render the file in a headless browser and assert each `<svg>` has child elements:
>
> 1. `preview_start` the `lead-magnets-preview` server (config in `.claude/launch.json`, port 8092, serves `content/learn-platform/lead-magnets/`).
> 2. `preview_eval` with this expression, where `<your-slug>` is the filename without extension:
>
>    ```javascript
>    (() => {
...
>
> 5. After validation passes, take one screenshot of a representative diagram (`preview_eval` to scroll to a `<figure>`, then `preview_screenshot`) so the user has visual confirmation in the conversation.
>
> If the gate fails, you must return to Phase 4 (regenerate SVG) and/or Phase 5 (fix assembly logic). Do not proceed to commit. Do not declare the bundle complete. The diagram count + rendering IS a publish gate, not an aspirational target.
>
...
**Phase 2 + 2.5 (sequential, 4 passes):** Draft each content type in this order, with its own Opus 4.8 generation call via the gateway, quality rubric self-critique, AND its own Opus 4.8 humanization pass before moving to the next type:
...
4. **Instagram script last**
   ...
   - **Run Phase 2.5 humanization on the approved script.** Use the spoken-delivery framing in the humanization prompt so it preserves natural cadence (contractions, sentence fragments, conversational beats).

**Phase 3 (verification, optional):** Verification applies if `--verify` is set. It runs on the humanized text from Phase 2.5, not the raw Opus 4.8 draft.
...
2. **Phase 1.5: KB Enrichment (REQUIRED)** — For each major finding, run the agentic Drive retrieval loop (Phase 1.5): search the KB for that finding's pathways/interventions/biomarkers, read the matching entity docs, and mine their crosstalk and evidence sections. The KB enrichment serves three purposes:
