...
const deckFile = path.resolve(
  args.find(a => !a.startsWith('--') && a.endsWith('.json')) || path.join(SKILL, 'slides.json'),
);
    ...
    for (const [k, v] of Object.entries(s)) {
      const scan = t =>
        BANNED.forEach(([re, name]) => {
          if (re.test(t)) hits.push(`slide ${i + 1} .${k}: ${name} → "${t}"`);
        });
      if (typeof v === 'string') scan(v);
      else if (Array.isArray(v))
        v.forEach(x =>
          scan(typeof x === 'string' ? x : (x?.title || '') + ' ' + (x?.detail || '')),
        );
    }
  });
  return hits;
...
const BG = {
  'green-gradient': 'graphics/web/green-gradient.jpg',
  'blue-gradient': 'graphics/web/blue-gradient.jpg',
  gradient: 'graphics/web/gradient.jpg',
  'gradient-portrait': 'graphics/web/gradient-portrait.jpg',
  cover: 'graphics/web/cover.jpg',
  'cover-2': 'graphics/web/cover-2.jpg',
  section: 'graphics/web/section.jpg',
  'mountain-forms-1': 'graphics/web/mountain-forms-1.jpg',
  'mountain-forms-2': 'graphics/web/mountain-forms-2.jpg',
  'pantone-glow': 'graphics/web/pantone-glow.jpg',
  'blu-gradient': 'graphics/web/blu-gradient.svg',
  mountain: 'graphics/mountain.svg',
  'gradient-white': 'graphics/web/gradient-white.jpg',
  'white-bg': 'graphics/web/white-bg.jpg',
  'white-gradient': 'graphics/web/white-gradient.jpg',
  'white-linear': 'graphics/web/white-linear.jpg',
  'mountain-line': 'graphics/web/mountain-line.jpg',
};
function bgSrc(bg) {
  if (!bg) return null;
  if (BG[bg]) return BG[bg];
  return bg.includes('/') ? bg : `graphics/web/${bg}${bg.includes('.') ? '' : '.jpg'}`;
}
function bgMissing(bg) {
  const s = bgSrc(bg);
  return s ? !existsSync(path.join(SKILL, 'assets', s)) : false;
}

async function main() {
  if (!existsSync(deckFile)) {
    console.error('Deck not found:', deckFile);
    process.exit(1);
  }
  ...
  const [w, h] = String(norm.size || '1080x1350')
    .split('x')
    .map(Number);
  ...
  const missingBg = (norm.slides || [])
    .map((s, i) => (s.bg && bgMissing(s.bg) ? `slide ${i + 1}: bg "${s.bg}"` : null))
    .filter(Boolean);
  if (missingBg.length) {
  ...
  let sprite = '';
  if (repoRoot)
    sprite = await readFile(path.join(repoRoot, 'design-system', 'assets', 'sprite.svg'), 'utf8')
      .then(s => s.replace(/style="[^"]*"/, 'style="position:absolute;width:0;height:0"'))
      .catch(() => '');
  ...
  const overflow = await page.$$eval('.stage', els =>
    els.map((el, i) => (el.scrollHeight > el.clientHeight + 2 ? i + 1 : null)).filter(Boolean),
  );
  if (overflow.length)
    console.warn(
      '⚠ content overflows on slide(s):',
      overflow.join(', '),
      '— shorten copy or reduce items.',
    );
  ...
  const imgs = await Promise.all(
    pngs.map(async p => `data:image/png;base64,${(await readFile(p)).toString('base64')}`),
  );
  const pdfHtml =
